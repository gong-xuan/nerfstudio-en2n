...
from en2n.pips.nets.pips import Pips
from en2n.match_utils import gather_and_avg_by_traj, match_by_pips, init_latents
...
class InstructPix2Pix(nn.Module):
    ...
    def edit_image_by_cor_batch(
        ...
        traj, traj_flat, resized_imgs = match_by_pips(self.pips_model, image_8batch, pips_height, pips_width,)
        traj_latent_flat = (traj[...,1]//8*pips_width//8 + traj[...,0]//8).long()[0, :len(image)]
        invalid_mask = (traj_latent_flat<0) + (traj_latent_flat>pips_height//8*pips_width//8-1)
        traj_latent_flat[traj_latent_flat<0] = 0
        traj_latent_flat[traj_latent_flat>pips_height//8*pips_width//8-1] = pips_height//8*pips_width//8-1

        # add noise
        if self.noise is None:
            self.noise = torch.randn_like(latents)
        # noise = init_latents(latents.shape, traj_latent_flat, latents.device)[0]
        latents = self.scheduler.add_noise(latents, self.noise, self.scheduler.timesteps[0])  # type: ignore
        ...
        for i, t in enumerate(self.scheduler.timesteps):
            ...
            latents = gather_and_avg_by_traj(latents, traj_latent_flat, invalid_mask,
                                             lambda x,y: self.cluster_func(x,y,progress))
